...
namespace AMA {
	...
	public:
		...
		bool operator>(const iProduct& ipr) const;
		int operator+=(int add);						// adds units to Product object and returns updated units on hand
	};
	std::ostream& operator<<(std::ostream& os, const iProduct& pr);	// insert a Product object into the ostream
	std::istream& operator>>(std::istream& is, iProduct& pr);		// extracts a Product object from the istream
	double operator+=(double& cost, const iProduct& pr);				// adds total cost of Product object to the double received and returns updated double
}
#endif
